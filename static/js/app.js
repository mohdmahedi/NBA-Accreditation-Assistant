/* app.js — NBA Accreditation Assistant */
'use strict';
...
const CRIT_COLORS = ['#3b82f6','#8b5cf6','#10b981','#f59e0b','#ef4444','#06b6d4','#ec4899','#f97316'];
...
function appendUserMsg(text) {
  ...
  row.innerHTML = `<div class="msg-avatar"><i class="bi bi-person-fill"></i></div>
...
function appendBotMsg(markdown, sources) {
  ...
  if (sources && sources.length) {
    src = '<div class="source-row">' +
      sources.map((_,i)=>`<span class="src-chip"><i class="bi bi-file-earmark-text me-1"></i>Source ${i+1}</span>`).join('') +
      '</div>';
  }
  row.innerHTML = `<div class="msg-avatar"><i class="bi bi-robot"></i></div>
    <div class="msg-bubble">${renderMd(markdown)}${src}</div>`;
  c.appendChild(row);
}
...
function showTyping() {
  ...
  r.innerHTML = `<div class="msg-avatar"><i class="bi bi-robot"></i></div>
...
async function clearHistory() {
  ...
  c.innerHTML = `<div class="welcome-message">
    ...
    <h4>Hello! I'm your NBA Accreditation Assistant.</h4>
    <p>Chat history cleared. Ask me anything about NBA accreditation.</p></div>`;
...
async function fetchCriteria() {
  try {
    criteriaData = await (await fetch(API.criteria)).json();
  } catch(_) {
    criteriaData = getBuiltinCriteria();
  }
  renderCriteriaCards();
  renderAccordion();
}
...
function getBuiltinCriteria() {
  ...
  const subs = [
    ['Institute Vision & Mission','PEO Correlation','CO-PEO Mapping'],
    ['Curriculum Design','Teaching-Learning','CO Definitions','CO-PO Mapping'],
    ['CO Attainment Direct','CO Attainment Indirect','PO/PSO Attainment','Mapping Strength'],
    ['Student Intake','Pass Percentage','Placements','Publications'],
    ['Faculty Qualifications','Faculty-Student Ratio','Research Activity','FDP Activities'],
    ['Classrooms','Laboratories','Computing Facilities','Library Resources'],
    ['Academic Improvement','Placement Improvement','CQI Actions','Feedback System'],
    ['First Year Performance','First Year Faculty'],
  ];
  const result = {};
  names.forEach((name, i) => {
    result['criterion_'+(i+1)] = {id:i+1, title:name, max_marks:MAX_MARKS[i], sub_criteria:subs[i]};
  });
  return result;
}
    ...
    col.innerHTML = `
      <div class="criterion-card">
        ...
        <div class="crit-meta">
          ...
          <input type="number" class="score-inp" id="score-${key}"
            min="0" max="${crit.max_marks}" value="0" placeholder="Score"
            onclick="event.stopPropagation()"
            oninput="updateScore('${key}',this.value)" />
        </div>
        <div class="crit-progress">
          <div class="crit-progress-bar" id="bar-${key}"
               style="width:0%;background:${CRIT_COLORS[idx]}"></div>
        </div>
        <div class="crit-subchecks">${subsHtml}</div>
        <button class="ask-crit-btn" onclick="askCriterion('${crit.title}')">
          <i class="bi bi-chat-dots me-1"></i>Ask AI about this criterion
        </button>
      </div>`;
    ...
    item.innerHTML = `
      <h2 class="accordion-header">
        <button class="accordion-button collapsed" type="button"
          data-bs-toggle="collapse" data-bs-target="#acc-${key}">
          <span style="width:10px;height:10px;border-radius:50%;background:${CRIT_COLORS[idx]};
            display:inline-block;margin-right:10px;flex-shrink:0;"></span>
          Criterion ${crit.id}: ${crit.title}
          <span class="badge ms-2"
            style="background:${CRIT_COLORS[idx]}25;color:${CRIT_COLORS[idx]};font-size:11px;">
            ${crit.max_marks} marks</span>
        </button>
      </h2>
      <div id="acc-${key}" class="accordion-collapse collapse">
        <div class="accordion-body">
          ...
          <button class="btn btn-sm btn-outline-primary mt-2"
            onclick="askCriterion('${crit.title}'); showTab('chat')">
...
function updateTotals() {
  ...
  if (bar) {
    ...
    bar.classList.add(pct < 40 ? 'bg-danger' : pct < 69 ? 'bg-warning' : 'bg-success');
  }
}
...
async function generateMapping() {
  ...
  out.innerHTML = `<div class="text-center py-5">
    ...
    <div class="text-muted">Generating CO-PO mapping via IBM Granite…</div>
    <div class="text-muted small mt-1">Retrieving context from NBA knowledge base</div></div>`;
  const btn = document.getElementById('generateBtn');
  if (btn) btn.disabled = true;
  try {
    const res  = await fetch(API.copo, {
      method:'POST', headers:{'Content-Type':'application/json'},
      body: JSON.stringify({course_name:courseName, course_outcomes:cos, num_psos:numPsos}),
    });
    const data = await res.json();
    if (data.error) { out.innerHTML = `<div class="text-danger p-3">${esc(data.error)}</div>`; return; }
    renderCopoResult(data, cos, numPsos);
  } catch(err) {
    out.innerHTML = `<div class="text-danger p-3">Network error: ${esc(err.message)}</div>`;
  } finally {
...
function renderCopoResult(data, cos, numPsos) {
  ...
  // Generate heuristic matrix (visual placeholder; real analysis in AI text)
  const matrix = cos.map((_,ci)=>
    cols.map((_,pi)=>{ const s=(ci*3+pi*7)%13; return s<2?0:s<5?1:s<9?2:3; })
  );
  ...
  out.innerHTML = `
    <div>
      <div class="d-flex justify-content-between align-items-center mb-3">
        <div>
          <strong>${esc(data.course_name)}</strong>
          <div class="text-muted small">${cos.length} COs &middot; ${POS.length} POs &middot; ${numPsos} PSOs</div>
        </div>
        <button class="copy-btn" onclick="copyMatrix()">
          <i class="bi bi-clipboard me-1"></i>Copy Table
        </button>
      </div>
      <div class="table-responsive">
        <table class="copo-table" id="copoMatrix">
          <thead><tr><th style="text-align:left">CO \\ PO</th>${headCols}</tr></thead>
          <tbody>${bodyRows}</tbody>
        </table>
      </div>
      <div class="analysis-box">${renderMd(data.analysis || 'No analysis returned.')}</div>
    </div>`;
}
...
function showToast(msg, type='info') {
  ...
  el.innerHTML = `<i class="bi bi-${icons[type]||'info-circle-fill'}" style="color:${colours[type]||'#3b82f6'};font-size:16px;"></i><span>${esc(msg)}</span>`;
  ctr.appendChild(el);
